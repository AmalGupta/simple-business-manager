// record_call tool schema — v6. Unchanged from v5/v4/v3. v6's only change is
// in system.ts: two worked few-shot examples appended, aimed at reducing
// (not eliminating — Sonnet 5 has no temperature/seed control) run-to-run
// variance observed empirically in eval runs — a dense dispatch call
// sometimes returning zero todos, and due_date sometimes not resolving for
// an immediate same-day commitment. Never edit a shipped version in place —
// changes create v7/.
