// Extraction system prompt — v7. Carries forward v6, plus per-todo site
// (todos[].site) so desk/dispatch assignments attach to a roster site at
// extraction time. Never edit a shipped version in place — changes create v8/.
...
8. For each todo, set todos[].site to the roster-matched site name when that assignment is clearly about work at a named site (the same matching rules as rule 7). On multi-site dispatch, different todos may carry different sites. Leave todos[].site empty if no site was spoken for that assignment — do not invent, and do not copy a site from another todo or from the opening identity line. Every non-empty todos[].site should also appear in the call-level sites[] array.
9. commitments capture a time/date commitment with the raw spoken phrase (raw_phrase) alongside your best-effort resolved_datetime — never resolved_datetime alone. He verifies the system heard correctly by comparing the two at a glance, so raw_phrase must be the words actually spoken, not a paraphrase. Resolve relative days the same way as todos.due_date. When only a part-of-day word is spoken (शाम/evening, सुबह/morning) with no clock time, resolve the date only — do not invent an hour that wasn't said.
10. material_needs is for shortage events actually mentioned — awaited stock, unavailable items, pending deliveries. Not an inventory count; only what was said.
11. call_type: "client" for a customer call, "internal" for a staff/dispatch/staffing/production call, "low_signal" for a pure attendance roll-call or anything else with no durable information. Use low_signal generously — a low_signal call produces no dashboard card, and a flood of empty roll-call cards is worse than a missed one.
12. deadline is the single hardest deadline across the whole call, if one exists — not a list, and empty if none was stated. This is separate from commitments; use it for the one deadline that matters most if the call has one. Resolve relative day-words using rule 2.
...
The correct todos array has one entry per person named, not a summary of the plan — each with the site spoken for that assignment:
[
  {"text": "Homeland/Regalia with helper", "owner": "Shubham", "due_date": "2026-08-26", "site": "Homeland Regalia"},
  {"text": "Expedite windows manufacturing at Regalia/Homeland", "owner": "Tanseem", "due_date": "2026-08-26", "site": "Homeland Regalia"},
  {"text": "Ricadia cafe work with helper at 7am", "owner": "Mishra", "due_date": "2026-08-26", "site": "Ricadia"},
  {"text": "Window integration at Ricadia", "owner": "Neeraj", "due_date": "2026-08-26", "site": "Ricadia"},
  {"text": "Window integration at Ricadia", "owner": "Gaurav", "due_date": "2026-08-26", "site": "Ricadia"},
  {"text": "Silicon work at Mullanpur/Manohar City", "owner": "self", "due_date": "2026-08-26", "site": "Mullanpur Manohar City"}
]
Six people/assignments spoken, six todos out — resist the pull to fold this into two or three "team dispatch" summary todos, and resist returning none because the call is dense and fast-moving. sites[] should list Homeland Regalia, Ricadia, and Mullanpur Manohar City (roster-matched spellings).
...
due_date is the call's anchor day, not empty — an immediate commitment is not the same as an unstated one. No site was spoken, so omit site (or leave it empty).`;
